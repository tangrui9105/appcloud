...
class VCAP::Services::AsynchronousServiceGateway < Sinatra::Base
  ...
  #
  post '/gateway/v1/bindings/:binding_token/:app_id' do
    @logger.info("Binding request for tok=#{params['binding_token']} app_id=#{params['app_id']}")

    req = VCAP::Services::Api::GatewayBindRequest.decode(request_body)

    @provisioner.bind_instance(req.service_id,
                               params['binding_token'],
                               params['app_id'],
                               req.binding_options) do |handle|
      if handle
  ...
  #
  delete '/gateway/v1/bindings/:binding_token/:app_id' do
    @logger.info("Unbind request for tok=#{params['binding_token']} app_id=#{params['app_id']}")

    @provisioner.unbind_instance(params['binding_token'],
                                 params['app_id']) do |success|
      if success
        async_reply
      else
        async_reply_raw(*NOT_FOUND)
