...
class VCAP::Services::Mysql::Provisioner
  ...
  def provision_service(version, plan, &blk)
    ...
    subscription = nil
    barrier = VCAP::Services::Mysql::Barrier.new(:timeout => 2, :callbacks => @nodes.length) do |responses|
  ...
  def provision_node(version, plan, mysql_nodes, blk)
    ...
    if node_with_most_storage
      ...
      timer = EM.add_timer(2) {@mysql_nats.unsubscribe(subscription)}
      ...
      @logger.warn("Could not find a mysql node to provision: #{provision_request} #{reply}")
    end
  ...
  def bind_instance(instance_id, binding_token, app_id, options, &blk)
    ...
    if svc
      ...
      @logger.debug("Binding MySQL instance #{instance_id} to handle #{handle['id']}")
    end
    blk.call(handle)
  end
